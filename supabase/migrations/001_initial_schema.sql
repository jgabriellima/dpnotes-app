-- Deep Research Notes - Initial Database Schema
...
-- ============================================
-- ANNOTATION_LABELS TABLE (Many-to-Many)
-- ============================================
...
-- ============================================
-- RLS POLICIES - ANNOTATION_LABELS
-- ============================================
...
CREATE POLICY "Users can create annotation labels in their documents"
  ON annotation_labels FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM annotations a
      JOIN documents d ON d.id = a.document_id
...
CREATE POLICY "Users can delete annotation labels in their documents"
  ON annotation_labels FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM annotations a
      JOIN documents d ON d.id = a.document_id
...
CREATE POLICY "Users can create audio in their annotations"
...
CREATE POLICY "Users can delete audio in their annotations"
...
-- Decrement label usage count
CREATE OR REPLACE FUNCTION decrement_label_usage()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE labels
...
CREATE TRIGGER decrement_label_usage_trigger
  AFTER DELETE ON annotation_labels
  FOR EACH ROW EXECUTE FUNCTION decrement_label_usage();

-- ============================================
-- SEED DATA - PREDEFINED LABELS
-- ============================================
